...
def test_permission_initialization():
    ...
    with app.app_context():
        ...
        print(f"✅ PASS: Created {created} permissions")
        ...
        if total != expected:
            print(f"❌ FAIL: Expected {expected} permissions, got {total}")
            return False

        print(f"✅ PASS: All {expected} permissions created")

        # Verify permission codes are unique
        codes = [p.code for p in db.session.query(Permission).all()]
        if len(codes) != len(set(codes)):
            print("❌ FAIL: Duplicate permission codes found")
            return False

        print("✅ PASS: All permission codes are unique")
...
def test_role_permission_assignment():
    ...
    with app.app_context():
        ...
        print(f"✅ PASS: Created {created} role-permission assignments")
        ...
        if admin_perms != total_perms:
            print(f"❌ FAIL: Admin should have all {total_perms} permissions, has {admin_perms}")
            return False

        print(f"✅ PASS: Admin has all {total_perms} permissions")
        ...
        if manager_perms >= admin_perms:
            print(f"❌ FAIL: Manager should have fewer permissions than admin")
            return False

        print(f"✅ PASS: Manager has {manager_perms} permissions (less than admin)")
        ...
        if cashier_perms >= manager_perms:
            print(f"❌ FAIL: Cashier should have fewer permissions than manager")
            return False

        print(f"✅ PASS: Cashier has {cashier_perms} permissions (minimal access)")
...
def test_user_permission_checking():
    ...
    with app.app_context():
        ...
        if not permission_service.user_has_permission(admin.id, "SYSTEM_ADMIN"):
            print("❌ FAIL: Admin should have SYSTEM_ADMIN permission")
            return False
        print("✅ PASS: Admin has SYSTEM_ADMIN permission")

        # Test manager does NOT have SYSTEM_ADMIN permission
        if permission_service.user_has_permission(manager.id, "SYSTEM_ADMIN"):
            print("❌ FAIL: Manager should NOT have SYSTEM_ADMIN permission")
            return False
        print("✅ PASS: Manager does not have SYSTEM_ADMIN permission")
        ...
        if not permission_service.user_has_permission(manager.id, "APPROVE_ADJUSTMENTS"):
            print("❌ FAIL: Manager should have APPROVE_ADJUSTMENTS permission")
            return False
        print("✅ PASS: Manager has APPROVE_ADJUSTMENTS permission")

        # Test cashier has CREATE_SALE permission
        if not permission_service.user_has_permission(cashier.id, "CREATE_SALE"):
            print("❌ FAIL: Cashier should have CREATE_SALE permission")
            return False
        print("✅ PASS: Cashier has CREATE_SALE permission")
        ...
        if permission_service.user_has_permission(cashier.id, "APPROVE_ADJUSTMENTS"):
            print("❌ FAIL: Cashier should NOT have APPROVE_ADJUSTMENTS permission")
            return False
        print("✅ PASS: Cashier does not have APPROVE_ADJUSTMENTS permission")
...
def test_security_event_logging():
    ...
    with app.app_context():
        ...
        try:
            ...
            print("✅ PASS: Permission check succeeded (CREATE_SALE granted)")
        except:
            print("❌ FAIL: Permission check should have succeeded")
            return False
        ...
        if not granted_event:
            print("❌ FAIL: PERMISSION_GRANTED event not logged")
            return False

        print("✅ PASS: PERMISSION_GRANTED event logged")
        ...
        try:
            ...
            print("❌ FAIL: Permission check should have failed")
            return False
        except permission_service.PermissionDeniedError:
            print("✅ PASS: Permission check correctly denied (SYSTEM_ADMIN)")
        ...
        if not denied_event:
            print("❌ FAIL: PERMISSION_DENIED event not logged")
            return False

        print("✅ PASS: PERMISSION_DENIED event logged")

        # Verify event details
        if denied_event.action != "SYSTEM_ADMIN":
            print(f"❌ FAIL: Event action wrong: {denied_event.action}")
            return False

        if "Missing permission" not in denied_event.reason:
            print(f"❌ FAIL: Event reason wrong: {denied_event.reason}")
            return False

        print("✅ PASS: Security event details correct")
...
def test_grant_revoke_permissions():
    ...
    with app.app_context():
        ...
        if permission_service.user_has_permission(cashier.id, "VOID_SALE"):
            print("❌ FAIL: Cashier should not have VOID_SALE permission initially")
            return False

        print("✅ PASS: Cashier does not have VOID_SALE initially")

        # Grant VOID_SALE to cashier role
        permission_service.grant_permission_to_role("cashier", "VOID_SALE")
        print("✅ PASS: Granted VOID_SALE to cashier role")

        # Verify cashier now has VOID_SALE
        if not permission_service.user_has_permission(cashier.id, "VOID_SALE"):
            print("❌ FAIL: Cashier should have VOID_SALE after grant")
            return False

        print("✅ PASS: Cashier now has VOID_SALE permission")

        # Revoke VOID_SALE from cashier role
        revoked = permission_service.revoke_permission_from_role("cashier", "VOID_SALE")

        if not revoked:
            print("❌ FAIL: Revocation should return True")
            return False

        print("✅ PASS: Revoked VOID_SALE from cashier role")

        # Verify cashier no longer has VOID_SALE
        if permission_service.user_has_permission(cashier.id, "VOID_SALE"):
            print("❌ FAIL: Cashier should not have VOID_SALE after revoke")
            return False

        print("✅ PASS: Cashier no longer has VOID_SALE permission")
...
def test_get_user_permissions():
    ...
    with app.app_context():
        ...
        if not isinstance(admin_perms, set):
            print(f"❌ FAIL: Should return a set, got {type(admin_perms)}")
            return False

        print(f"✅ PASS: Returns set of permissions")

        # Verify admin has many permissions
        if len(admin_perms) < 20:
            print(f"❌ FAIL: Admin should have many permissions, got {len(admin_perms)}")
            return False

        print(f"✅ PASS: Admin has {len(admin_perms)} permissions")
        ...
        for perm in required_perms:
            if perm not in admin_perms:
                print(f"❌ FAIL: Admin should have {perm} permission")
                return False

        print(f"✅ PASS: All required permissions present")
...
def test_user_with_no_roles():
    ...
    with app.app_context():
        ...
        if len(user_perms) != 0:
            print(f"❌ FAIL: User with no roles should have 0 permissions, has {len(user_perms)}")
            return False

        print("✅ PASS: User with no roles has no permissions")

        # Verify permission check fails
        if permission_service.user_has_permission(user.id, "CREATE_SALE"):
            print("❌ FAIL: User with no roles should not have any permissions")
            return False

        print("✅ PASS: Permission checks correctly fail for user with no roles")
...
def test_permission_categories():
    ...
    with app.app_context():
        ...
        for category in categories:
            ...
            if len(perms) == 0:
                print(f"❌ FAIL: Category {category} has no permissions")
                return False

            print(f"✅ PASS: Category {category} has {len(perms)} permissions")
